---
title: "911 5b-17 MAY DAY"
description: |
  Tweets and Word Cloud for "May Day" - 911 on Fox.
author:
...
tweets17 <- read_csv("tweets17.csv")
head(tweets17)

```

### Separate the Data into only post-5b 17 tweets.
...
nrow(workingTweets17)

#format = "%Y-%m-%d %H:%M:%s"
# show start date march 29 12 midnight Greenwich Mean Time
start_date <- as.POSIXct('2022-05-09 00:00:00', tz="UTC")

workingTweets17 <- workingTweets17 %>% filter(created_at >= start_date)
...
wordcloud2(data=top43, color = "random-dark", .75)
...
```{r}
my_colors911 <- rainbow(nrow(top43), start = 0.7) # check ?rainbow for more infos

#, start = 0.1
#shuffle = F,

wordcloud2(top43, minRotation = 0, maxRotation = 0, color = my_colors911, size = .75)
```
